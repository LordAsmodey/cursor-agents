...
|-------|------|----------------------|
| **architect** | Full-stack design: structure, contracts, DTOs, constraints for Orchestrator | Design phase; optional for trivial features |
| **orchestrator** | Decompose feature into tasks, dependencies, execution plan | After (optional) Architect |
...
- **Does:** Analyzes requirements, evaluates codebase patterns, defines frontend and backend approach, specifies boundaries, DTOs, API contracts, file structure, state management, steering rules, constraints for Orchestrator, risks, optional ADR.
...
- **Does:** Analyzes feature request, breaks into atomic tasks, defines dependencies and execution order, assigns assignees, produces execution plan.
- **Does not:** Implement code, review code, make architecture decisions.
- **Output:** List of tasks (with id, title, scope, depends_on, acceptance_criteria, assignee, parallel_group) and a step-by-step execution plan.
- **Rules:** No circular dependencies; max 3 rework iterations per task then escalate; scope must not overlap (or tasks strictly sequenced).
...
3. In Architect and Orchestrator instructions/outputs, use the new assignee (e.g. `backend-worker`, `backend-reviewer`) for backend tasks.
